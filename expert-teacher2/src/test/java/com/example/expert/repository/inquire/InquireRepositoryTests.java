...
public class InquireRepositoryTests {
    ...
    public void answerSaveTest(){
//        Answer answer = Answer.builder().answerContents("비밀번호 변경은 한 달에 한 번만 가능합니다.").build();
        Answer answer = Answer.builder().answerContents("혹시 한 달이 지낫음에도 불구하고 비밀번호 변경이 되지 않으면 고객센터로 문의부탁드립니다.").build();
        questionRepository.findById(274L).ifPresent(answer::setQuestion);
        answerRepository.save(answer);
    }
